...
def find_split(
    ...
    for i in range(num_features):
        ...
        for j, entry in enumerate(sorted[1:]):
            ...
            if label != current_label:
                ...
                entropy_gain = math_utils.get_entropy_gain(y_dataset_all, y_dataset_above, y_dataset_below)
    ...
    return (DecisionTreeNode(function=lambda x: x[split_feature] < split_value,
        node_label="x[{}] < {}".format(split_feature, split_value))
        , best_above_split
        , best_below_split
        , used_splits)
...
def decision_tree_learning(
    ...
    if len(unique_ys) == 1:
        ...
        new_node, left_split, right_split, new_split_features = find_split(training_dataset, split_features)
        left_branch, left_depth = decision_tree_learning(left_split, depth + 1, new_split_features)
        right_branch, right_depth = decision_tree_learning(right_split, depth + 1, new_split_features)
...
if __name__ == "__main__":
    dataset = file_utils.read_dataset("./intro2ML-coursework1/wifi_db/clean_dataset.txt")
    train, test, validation = file_utils.split_dataset(dataset, [0.6, 0.8])
    tree, depth = decision_tree_learning(train, 10, [])
    tree.print_tree(0)
